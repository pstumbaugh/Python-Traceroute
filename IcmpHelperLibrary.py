...
def getAvgRTT():  # returns the average RTT. If no RTT's, returns 0
...
def printRTTs():
    ...
    print("     Avg RTT: ", round(average, 2),
          " *based on exact RTT times (not rounded)")
...
# 1. Update the __validateIcmpReplyPacketWithOriginalPingData() function
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            if self.getPacketSequenceNumber() != icmpReplyPacket.getIcmpSequenceNumber():
                checkFlag = False
                print("Packet Sequence number expected: ", self.getPacketSequenceNumber(
                ), ", BUT - Packet Sequence number received: ", icmpReplyPacket.getIcmpSequenceNumber())
            ...
            if self.getPacketIdentifier() != icmpReplyPacket.getIcmpIdentifier():
                checkFlag = False
                print("Packet Identifier expected: ", self.getPacketIdentifier(
                ), ", BUT - Packet Identifier received: ", icmpReplyPacket.getIcmpIdentifier())
            ...
            if self.getDataRaw() != icmpReplyPacket.getIcmpData():
                checkFlag = False
                print("Raw Data expected: ", self.getDataRaw(),
                      ", BUT - Raw Data received: ", icmpReplyPacket.getDataRaw())
...
# is the foundation for such functionality.
        ...
        def printResultToConsole(self, ttl, timeReceived, addr, original):
            ...
            if self.getIcmpIdentifier_isValid == False:
                print("ICMP Identifier invalid. Received: ",
                      self.getIcmpIdentifier(), "BUT - expected ", original.getPacketIdentifier())
            if self.getIcmpSequenceNumber_isValid == False:
                print("ICMP Sequence Number invalid. Received: ",
                      self.getIcmpSequenceNumber(), "BUT - expected ", original.getPacketSequenceNumber())
            if self.getIcmpRawData_isValid == False:
                print("ICMP Raw Data invalid. Received: ",
                      self.getIcmpData(), "BUT - expected ", original.getDataRaw())
